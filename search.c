        ...
        if (sameLocation(thisLocation->x, thisLocation->y, game->goal->x, game->goal->y))
        {
            ...
            for (int i = 0; i < len; i += 1)
            {
                ...
                if (!inClosed(successor, closed))
                {
                    ...
                    FringeNode *fn = newFringeNode(successor, allActions, costOfActions);
